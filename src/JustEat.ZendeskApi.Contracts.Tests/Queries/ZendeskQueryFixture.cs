...
namespace JustEat.ZendeskApi.Contracts.Tests.Queries
{
    public class ZendeskQueryFixture
    {
        ...
        [Test]
        public void CalledWithNegativeCustomFieldsTwice_BuildsQuery()
        {
            // Given
            var query = new ZendeskQuery<Organization>();
            query.WithNegativeCustomFilter("name", "cheese factory");
            query.WithNegativeCustomFilter("name", "meat factory");
        ...
        [Test]
        public void CalledWithPositiveAndNegativeCustomFields_BuildsQuery()
        {
            // Given
            var query = new ZendeskQuery<Organization>();
            query.WithCustomFilter("name", "cheese factory");
            ...
            // Then
            Assert.That(queryString, Is.EqualTo("query=type:organization+name:cheese factory+-name:meat factory&sort_by=created_at&sort_order=desc&page=1&per_page=15"));
        }
    }
}
